...
class Test_mvnpy(unittest.TestCase):
    def setUp(self):
        ...
        self.cache_states_dir = reduce(lambda acc, curr: os.path.join(acc, curr),
                                       [os.getcwd(), 'static_files', 'cache_states'])
    ...
    def set_up_commit_env(self, github, commit, repo_path=''):
        g_repo = git.Repo(
            path=self.get_default_repo_path(github) if repo_path == '' else repo_path
        )
        g_repo.git.add('.')
        g_repo.git.checkout(commit, '-f')

    def set_up_cached_state(self, github, commit, cached_state_path, repo_path=''):
        repo_path = self.get_default_repo_path(github) if repo_path == '' else repo_path
        self.set_up_commit_env(github=github, commit=commit, repo_path=repo_path)
        git.Repo(path=repo_path).git.apply(cached_state_path, '--whitespace=nowarn')

    def get_default_repo_path(self, github):
        return reduce(lambda acc, curr: os.path.join(acc, curr),
                      [os.getcwd(), 'static_files', os.path.basename(github)])

    def test_generate_comp_error_paths(self):
        def assert_path_in_errors(path, errors_paths):
            self.assertTrue(path in errors_paths)

        github = 'https://github.com/apache/commons-lang'
        repo_path = self.get_default_repo_path(github)
        self.set_up_cached_state(
            github=github,
            commit='2d9a9ae59fd3b753e05903f63641eff1203f1bd1',
            cached_state_path=os.path.join(self.cache_states_dir,
                                           'comp_errors_with_sub_path_2d9a9_09-16-2019--10-45-10.patch'),
            repo_path=repo_path
        )
        repo = Repo.Repo(repo_path)
        repo.clean()
        errors = mvn.get_compilation_errors(
            mvn.get_compilation_error_report(
                repo.test_compile()
            )
        )
    ...
    def test_change_surefire_ver_2(self):
        ...
        for pom in poms:
            print('#### checking ' + pom + ' ######')
            if (os.path.normcase(
                    r'C:\Users\user\Code\Python\BugMiner\mvn_parsers\static_files\tika\tika-dotnet\pom.xml') == os.path.normcase(
                pom)):
                print('#### passing ' + pom + ' ######')
                continue
            module_path = os.path.abspath(os.path.join(pom, os.pardir))
    ...
    def test_setup_tracer(self):
        ...
        with open(expected_paths_path, 'rb') as paths:
            ...
            self.assertEqual(lines[0].replace('\n', '').replace('\r', ''),
                             os.path.join(os.environ['USERPROFILE'], r'.m2\repository'))
            self.assertEqual(lines[1], repo.repo_dir)
        with open(os.path.join(repo.repo_dir, 'pom.xml'), 'rb') as pom:
            lines = pom.readlines()
            self.assertTrue('<argLine>-javaagent:{}={}</argLine>'.format(expected_agent_path, expected_paths_path),
                            os.path.join(os.environ['USERPROFILE'], r'.m2\repository'))
    ...
    def test_set_pom_tag_1(self):
        ...
        repo.set_pom_tag(xquery=r"./build/plugins/plugin[artifactId = 'maven-surefire-plugin']/version", module=module,
                         create_if_not_exist=True, value=expected_version)
        root = ET.parse(pom).getroot()
        xmlns, _ = mvn.tag_uri_and_name(root)
        surfire_tag_singelton = root.findall(
            r"{}build/{}plugins/{}plugin[{}artifactId='maven-surefire-plugin']/{}version".format(xmlns, xmlns, xmlns,
                                                                                                 xmlns, xmlns))
    ...
    def test_set_pom_tag_2(self):
        ...
        root = ET.parse(pom).getroot()
        xmlns, _ = mvn.tag_uri_and_name(root)
        junit_tag_singelton = root.findall(
            r"{}dependencyManagement/{}dependencies/{}dependency[{}artifactId='junit']/{}version".format(xmlns, xmlns,
                                                                                                         xmlns, xmlns,
                                                                                                         xmlns))
